...
export default async function ReportesPage() {
  ...
  const [{ data: payments }, { data: tenants }, { data: owners }, { data: payouts }] = await Promise.all([
    ...
    supabase.from('owner_payouts').select('*, owner:owners(full_name), property:properties(name)').order('period_year', { ascending: false }).order('period_month', { ascending: false }),
  ])
